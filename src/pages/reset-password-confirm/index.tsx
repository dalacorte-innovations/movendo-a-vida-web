  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${configBackendConnection.baseURL}/${endpoints.resetPasswordConfirm}${uidb64}/${token}/`,
        {
          method: "POST",
      ...
      <div className="flex w-full h-screen">
        ...
        <div className="hidden md:flex md:w-1/2 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center w-full h-full p-12 text-white">
            ...
            >
              <h1 className="text-4xl md:text-5xl font-bold mb-6">{t("Crie uma nova senha")}</h1>
              <p className={`text-xl md:text-2xl mb-8 ${darkMode ? "text-pink-200" : "text-blue-200"} max-w-md`}>
                {t("Defina uma nova senha segura para sua conta.")}
              </p>
              <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-md rounded-lg px-4 py-3 max-w-md">
                ...
                <span>{t("Use uma combinação de letras, números e símbolos para maior segurança")}</span>
              </div>
            </div>
          </div>
        </div>

        {/* Right side - reset password confirm form */}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <div className="flex items-center mb-2">
                    <MdLock className={`${darkMode ? "text-pink-500" : "text-blue-500"} mr-2`} size={20} />
