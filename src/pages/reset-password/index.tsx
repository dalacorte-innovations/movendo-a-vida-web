...
const ResetPasswordPage = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center ${darkMode ? "bg-[#0F172A]" : "bg-[#f0f7ff]"} font-metropolis relative overflow-hidden`}>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center w-full h-full p-12 text-white">
            <div className={`transform transition-all duration-1000 ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
              <h1 className="text-4xl md:text-5xl font-bold mb-6">
                {t('Recupere seu acesso')}
              </h1>
              <p className={`text-xl md:text-2xl mb-8 ${darkMode ? "text-pink-200" : "text-blue-200"} max-w-md`}>
                {t('Enviaremos instruções para redefinir sua senha para o seu e-mail.')}
              </p>
              <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-md rounded-lg px-4 py-3 max-w-md">
                <div className={`w-2 h-2 rounded-full ${darkMode ? "bg-pink-400" : "bg-green-400"}`}></div>
                <span>{t('Verifique sua caixa de entrada e spam após solicitar a redefinição')}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <div className="flex items-center mb-2">
                    ...
                    <label htmlFor="email" className={`block ${darkMode ? "text-white" : "text-slate-800"} text-sm`}>
                      {t('E-mail')}
                    </label>
                  </div>
